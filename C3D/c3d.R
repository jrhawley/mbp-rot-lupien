...
motifFile <- args[11] # a file with a list of motifs
fimo_pcut <- as.numeric(args[12]) # p-value cut-off for FIMO
fimo_qcut <- as.numeric(args[13]) # q-value cut-off for FIMO
figures <- tolower(args[14]) # 'y' if you want interaction landscapes outputted
figureWidth <- args[15] # flanking bps from anchor to display on figure
zoom <- args[16] # how many flanking bps for zoomed landscapes
colour <- args[17] # colours for the interaction plots
tracks <- args[18] # 'y' if you want tracks outputted
sampleName <- args[19]
trackNumber <- as.numeric(args[20])
numSamples <- as.numeric(args[21])
date <- args[22] # timestamp for output pdf
workingDir <- args[23] # working directory
...
if (tracks=="y") {
  if (length(TFs) > 0) {
    ...
    for (t in 1:length(TFs)) { # write motifs to track if motifFile was passed
      ...
      trackHeader <- paste("track name=\"",TFs[t],
                           "\" description=\" \" visibility=1 color=",getRGB(t, motifPal)," db=hg19", sep="")
  ...
  for (r in 1:length(anchor.bed)) {
    ...
    file <- paste(outDir,"/",genes[r],".anchor", sep="")
    ...
    trackHeader <- paste("track type=bedGraph name=\"Anchor\" description=\"",gsub("_.*", "", genes[r]),
                         "\" visibility=full color=0,0,0 altColor=0,0,0 viewLimits=0:1 autoScale=off gridDefault=on db=hg19", sep="")
    ...
    trackHeader <- paste("track type=bedGraph name=\"",sampleName,"\" description=\" \" visibility=full color=",getRGB(trackNumber, trackPal), 
                         " altColor=0,0,0 viewLimits=0:1 autoScale=off gridDefault=on yLineMark=",rcut," yLineOnOff=on db=hg19", sep="")
    junk <- cat(trackHeader, file=file, append=FALSE, sep="\n")
